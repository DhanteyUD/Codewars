 ...
 * is published and each month he is the last on the list which means he is
 * the heaviest.
 *
 ...
 *
 * 180 is before 90 since, having the same "weight" (9), it comes before as
 * a string.
 *
...
function orderWeight(s) {
  return s
    ...
    .sort(
      (a, b) =>
        a.split('').reduce((pv, cv) => pv + +cv, 0) -
          b.split('').reduce((pv, cv) => pv + +cv, 0) || (a > b ? 1 : -1)
    )
    .join(' ');
}
